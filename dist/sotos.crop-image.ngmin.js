...
angular.module('sotos.crop-image').directive('imageCrop', [
  '$window',
  function ($window) {
    return {
      restrict: 'E',
        ...
        function ($scope) {
          ...
          SelectionCrop.prototype.drawWaterMarkImage = function () {
            ...
            if (this.w > 0 && this.h > 0) {
              ...
              srcCanvasCtx.drawImage(watermarkImage, (mainCanvas.width-this.w)* ratio_width, (mainCanvas.height-this.h)* ratio_width, this.w * ratio_width, this.h * ratio_width);
          ...
          this.getImage = function () {
            if ($scope.cropOptions.viewShowCropTool) {
              ...
              //close the crop tool
              $scope.cropOptions.viewShowCropTool = false;  // $scope.$apply();
            }
          };
          ...
          var hPer = null;
          //check
          if ($scope.cropOptions.viewSizeWidth.substring($scope.cropOptions.viewSizeWidth.length - 1) === '%' || $scope.cropOptions.viewSizeHeight.substring($scope.cropOptions.viewSizeHeight.length - 1) === '%') {
          ...
          $scope.$on('cropImageShow', function () {
            console.log(srcCanvas.toDataURL(imageType));
            $scope.imageOut = srcCanvas.toDataURL(imageType);
          });
...
;
angular.module('sotos.crop-image').directive('editCrop', [
  '$window',
  function ($window) {
    return {
      require: '^imageCrop',
